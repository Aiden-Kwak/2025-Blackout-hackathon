...
def generate_response_from_db(question, similar_messages):
    ...
    prompt = f"""
    당신은 전문적인 글 작성 어시스턴트입니다. 다음의 질문과 관련된 메시지를 바탕으로 명확하고 간결하며 블로그에 적합한 답변을 작성하세요. 
    답변은 HTML 형식으로 출력됩니다.
    1. 질문에 대한 답변은 <h1>, <p>, <ul>과 같은 HTML 태그를 사용해 계층적으로 구성합니다.
    2. 필요 시, 추가적인 세부 정보를 <p> 태그로 작성하세요.
    3. 각 관련된 메시지는 <ul> 리스트로 정리하며, 링크가 포함된 경우 "참고 링크" 섹션을 별도로 추가합니다.
    4. 블로그의 독자가 쉽게 이해할 수 있도록 가독성을 우선시하세요.
    5. 답변을 ```html 코드블럭으로 감싸지 마세요.
    ...
    반드시 위 관련 메시지를 바탕으로 답변을 작성하세요.
    절대 기존 외부 정보나 이미 아는 정보를 사용하지 말고, 관련한 메시지들이 없다고 생각한다면 거짓을 말하지 않고 솔직하게 관련이 없다고 답할 것.
    """
    client = OpenAI(api_key=openai_api_key) 
    try:
